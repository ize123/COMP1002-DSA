...
public class DSAGraph
{
   ...
   public void removeEdge(String label1, String label2, int distance)
   {
      Iterator iter = edges.iterator();
      while(iter.hasNext())
      {
         DSAGraphEdge edge = (DSAGraphEdge)iter.next();
         if(((DSAGraphVertex)edge.getFrom()).getLabel().equals(label1) && ((DSAGraphVertex)edge.getTo()).getLabel().equals(label2) && edge.getValue() == distance)
         {
            edges.removeAt(edge);
         }
      }
   ...
   private void printEdge(DSAGraphEdge edge)
   {
      Iterator iter = edge.security.iterator();
      String secString = "";
      while(iter.hasNext())
   ...
   public void BFSSearch(String src, String dest)
   {
      int size = getVertexCount();
   ...
   public void generateRoute(String src, String dest)
   {
      DSAGraphVertex srcNode = getVertex(src);
      DSALinkedList list = getAdjacent(src);

      for(Object o : list)
      {
         DSAGraphVertex vert = (DSAGraphVertex) o;
         if(vert.getLabel().equals(dest))
         {
            Iterator iter = edges.iterator();
            while(iter.hasNext())
            {
               DSAGraphEdge edge = (DSAGraphEdge)iter.next();
               if(((DSAGraphVertex)edge.getTo()).getLabel().equals(src) && ((DSAGraphVertex)edge.getFrom()).getLabel().equals(dest))
               {
                  printEdge(edge);
               }
            }
   ...
   public void displayWorld()
   {
      ...
      {
         DSAGraphVertex vert = (DSAGraphVertex) o;
         System.out.println("LOCATION NAME: [" + vert.getLabel() + "]");
         ...
         {
            DSAGraphEdge edge = (DSAGraphEdge)iter.next();
            if(edge.getFrom() == vert)
            {
               Iterator iter3 = edge.security.iterator();
               String secString = "";
               while(iter3.hasNext())
               {
                  secString += iter3.next().toString() + " ";
               }
               ...
               System.out.println(((DSAGraphVertex)edge.getFrom()).getLabel() + " ---- " + edge.getValue() + " ---> " +((DSAGraphVertex)edge.getTo()).getLabel() + "\tSecurity: " + secString + "\tBarriers: " + barString);
            }
         }
      }
   }
